...
// SINGLE UNIFIED Timeline Filter Implementation
let appliedFilter = 'all'; // Track the currently applied filter
...
function renderTimelineFilters() {
  ...
  if (!filtersDiv) return; // Safety check
  
  // Clear any existing filter buttons to avoid duplication
  filtersDiv.innerHTML = '';
  
  // Create filter buttons container and refresh button
  const buttonsHTML = `
    <div class="filter-buttons-container">
      <button class="timeline-filter-btn${appliedFilter==='all' ? ' active' : ''}" data-filter="all">All</button>
      <button class="timeline-filter-btn${appliedFilter==='chat' ? ' active' : ''}" data-filter="chat">Chat</button>
      <button class="timeline-filter-btn${appliedFilter==='command' ? ' active' : ''}" data-filter="command">Command</button>
    </div>
    <button class="message-refresh-btn" title="Refresh messages"><i class="fas fa-sync-alt"></i></button>
  `;
  
  filtersDiv.innerHTML = buttonsHTML;
  ...
  filtersDiv.querySelectorAll('.timeline-filter-btn').forEach(btn => {
    btn.onclick = () => {
      ...
      applyMessageFilter(newFilter);
    };
  });
  
  // Handle refresh button click
  const refreshBtn = filtersDiv.querySelector('.message-refresh-btn');
  if (refreshBtn) {
    refreshBtn.onclick = () => {
      refreshBtn.classList.add('spinning');
      loadUnifiedMessageTimeline(1, 50, true); // Force refresh
      setTimeout(() => {
...
// SINGLE, ROBUST filter implementation that works across all components
function applyMessageFilter(filterType) {
  // Make function available immediately for this run and future calls
  window.applyMessageFilter = applyMessageFilter;
  console.log(`[FILTER] Applying filter: ${filterType} to all message elements`);
  
  // Force store to update regardless of where the filter was initiated from
  appliedFilter = filterType;
  timelineFilter = filterType;
  window.timelineFilter = filterType;
  if (window.messagesStore && window.messagesStore.setState) {
    try {
      window.messagesStore.setState({ filter: filterType });
    } catch (e) {
      console.error('Error updating message store:', e);
    }
  }
  
  // Apply visual changes to ALL filter buttons across all components
  // This catches both the app.js buttons and MessageTimeline.jsx buttons
  const allButtons = document.querySelectorAll('.timeline-filter-btn');
  ...
  // Enhanced Direct DOM-based filtering that works even if no store exists
  filterMessageElementsDirectly(filterType);
  
  // Inform any other components that care about filter changes
  setTimeout(() => {
    const filterEvent = new CustomEvent('nexus-filter-changed', {
...
// Separate function to directly filter message elements via DOM manipulation
async function filterMessageElementsDirectly(filterType) {
  // Find ALL message elements in the DOM (not just ones in the store)
  const messageEls = document.querySelectorAll('.msg-item');
  console.log(`[FILTER] Found ${messageEls.length} message elements to filter`);
  
  // Safety check - if no messages, nothing to do
  if (!messageEls.length) return;
  
  try {
    // Import the shared message classifier utility
    const { applyFilterToElements } = await import('./utils/messageClassifier.js');
    
    // Apply filtering using the shared utility
    const { chatCount, commandCount, otherCount } = applyFilterToElements(messageEls, filterType);
    
    console.log(`[FILTER] Results - Chat: ${chatCount}, Command: ${commandCount}, Other: ${otherCount}`);
  } catch (error) {
    console.error('[FILTER] Error using shared message classifier, falling back to legacy method:', error);
    
    // Legacy fallback implementation
    legacyFilterImplementation(messageEls, filterType);
  }
}

// Legacy implementation as fallback for backward compatibility
function legacyFilterImplementation(messageEls, filterType) {
  // Track counts for debugging
  let chatCount = 0;
  let commandCount = 0;
  let otherCount = 0;
  
  // Inspect each DOM message element
  messageEls.forEach(el => {
    const classList = Array.from(el.classList);
    
    // Enhanced message type detection
    // A message is considered 'chat' if ANY of these conditions are true
    const isChat = classList.includes('msg-chat') || 
                 classList.includes('msg-user') || 
                 el.querySelector('.msg-role i.fa-user') != null || // Has user icon
                 (classList.includes('msg-assistant') && !classList.includes('msg-command'));
    
    // A message is a command if it has command class OR contains command in content
    const isCommand = classList.includes('msg-command') || 
                    el.querySelector('.msg-type.msg-command') != null;
    
    // Apply visibility based on the selected filter
    if (filterType === 'all') {
  ...
  console.log(`[FILTER] Legacy Results - Chat: ${chatCount}, Command: ${commandCount}, Other: ${otherCount}`);
}
...
function handleTaskCompletion(taskId, status, result = {}) {
  ...
  outputsDiv.innerHTML = `
    ...
    ${result.nexusReportUrl ? `<a href="${result.nexusReportUrl}" target="_blank" class="btn btn-primary btn-sm mt-2">View Nexus Report</a>` : ''}
  `;
...
function handleHistoryCardClick(e) {
  ...
  fetch(`/history/${taskId}`, { credentials: 'include', headers: { 'Content-Type': 'application/json','X-Requested-With': 'XMLHttpRequest' }, })
    ...
    .then(item => {
      ...
      const { command, url, timestamp, result = {}, intermediateResults = [], landingReportUrl: topLanding, nexusReportUrl: topMidscene, runReport: topRun, errorReportUrl } = item;
  ...
  || (result && result.error ? `<span style='color: var(--danger);'>${result.error}</span>` : 'No summary available');
      ...
      if (topMidscene) {
        reportLinksHtml += `
          <a href="${topMidscene}" target="_blank" class="btn btn-primary btn-sm">View Nexus Report</a>
        `;
      }
      if (topRun) {
        reportLinksHtml += `
...
// Filter functionality has been migrated to messagesStore.applyFilter
// The store initializes on page load and handles all filter operations
