...
/**
 * GET /tasks/:id/yaml-execution
 * Get YAML map execution details for a specific task
 */
router.get('/:id/yaml-execution', requireAuth, async (req, res) => {
  try {
    ...
    if (!task.yamlMapId) {
      return res.status(400).json({ success: false, error: 'Task is not a YAML map execution' });
    }
    
    // Get YAML map details if available
    let yamlMap = null;
    try {
      const YamlMap = (await import('../models/YamlMap.js')).default;
      yamlMap = await YamlMap.findById(task.yamlMapId);
    } catch (err) {
      console.error('Error fetching YAML map:', err);
    }
    
    return res.json({
      success: true,
      task: stripLargeFields(task.toJSON ? task.toJSON() : task),
      yamlMap: yamlMap ? stripLargeFields(yamlMap.toJSON ? yamlMap.toJSON() : yamlMap) : null
    });
  } catch (err) {
    logger.error('Error retrieving YAML execution details:', err);
    return res.status(500).json({ success: false, error: err.message });
  }
});
...
router.get('/:id/report', requireAuth, async (req, res) => {
  try {
    ...
    if (!task.reportPath) {
      return res.status(404).json({ success: false, error: 'No report available for this task' });
    }
    ...
    return res.json({
      ...
      task: {
        id: task._id,
        yamlMapId: task.yamlMapId,
        yamlMapName: task.yamlMapName,
        status: task.status,
        startTime: task.startTime,
        endTime: task.endTime
      },
      reportPath: task.reportPath,
      reportUrl: `/reports/${task._id}`
    });
  } catch (err) {
    logger.error('Error retrieving task report:', err);
    return res.status(500).json({ success: false, error: err.message });
  }
})

// Import the YAML map ID extraction utility
import { extractYamlMapIdFromPrompt } from '../utils/yamlProcessor.js';
...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    // Check for YAML map references in the command
    const yamlMapId = extractYamlMapIdFromPrompt(command);
    
    // If a YAML map ID was found, look up the YAML map to validate it
    let yamlMapName = null;
    let yamlMapTags = [];
    
    if (yamlMapId) {
      // Import the YamlMap model here to avoid circular dependencies
      const YamlMap = (await import('../models/YamlMap.js')).default;
      
      // Check if the YAML map exists
      const yamlMap = await YamlMap.findById(yamlMapId);
      if (!yamlMap) {
        return res.status(404).json({ 
          success: false, 
          error: `YAML map with ID ${yamlMapId} not found` 
        });
      }
      
      // Check if user has access (owner or public map)
      if (yamlMap.userId !== req.session.user && !yamlMap.isPublic) {
        return res.status(403).json({ 
          success: false, 
          error: 'You do not have permission to use this YAML map' 
        });
      }
      
      yamlMapName = yamlMap.name;
      yamlMapTags = yamlMap.tags || [];
    }
