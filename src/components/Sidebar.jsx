  ...
  function createMapCard(map, isPublic) {
    ...
    if (contentPart) {
      ...
      contentPart.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        console.log('[Sidebar] YAML map item clicked, opening viewer for map ID:', map._id);
        
        try {
          // First try to use the viewer if available
          if (window.YamlMapViewer && typeof window.YamlMapViewer.open === 'function') {
            console.log('[Sidebar] Opening with YamlMapViewer.open');
