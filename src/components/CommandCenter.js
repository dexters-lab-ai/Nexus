...
export function CommandCenter(props = {}) {
    ...
    activeTasks.forEach(task => {
      ...
      taskEl.innerHTML = `
        ...
        <div class="task-details">
          <h4>${escapeHtml(task.name || 'Unnamed Task')}</h4>
          <p>${escapeHtml(task.description || 'No description')}</p>
          <button class="btn btn-sm btn-danger cancel-task" data-task-id="${task._id}">
      ...
      // Add cancel handler
      const cancelBtn = taskEl.querySelector('.cancel-task');
      if (cancelBtn) {
        cancelBtn.addEventListener('click', async (e) => {
          ...
          try {
            ...
            // The store will be updated via subscription
          } catch (error) {
  ...
  // Initialize active tasks display
  updateActiveTasks();
  
  // Subscribe to task updates
  const unsubscribeFromTasks = tasksStore.subscribe(updateActiveTasks);
  ...
  container.destroy = () => {
    ...
    if (nliForm) nliForm.removeEventListener('submit', null);
  };
