...
import { initYamlMapsIntegration } from './components/yaml-map-integration.js';
...
export function initializeModernUI(options = {}) {
  ...
  function setupEventHandlers() {
    ...
    // Handle command execution events (from history replay)
    eventBus.on('execute-command', (data) => {
      const { command, isYamlTask = false } = data;
      console.log(`Received execute-command event for ${isYamlTask ? 'YAML task' : 'direct text command'}:`, command);
      
      // Show notification that a command is being replayed
      eventBus.emit('notification', {
        title: `Replaying ${isYamlTask ? 'YAML Task' : 'Command'}`,
        message: `Replaying: ${command.length > 40 ? command.substring(0, 37) + '...' : command}`,
        type: 'info',
        duration: 3000,
        icon: `<i class="fas fa-${isYamlTask ? 'map' : 'history'}"></i>`
      });
      
      // Get the command input element and submit button
      // Try multiple possible selectors for different UI versions
      const inputSelectors = ['#unified-input', '#command-input', '#nli-input', '.command-input', '.nli-input', 'textarea[placeholder*="command"]'];
      const buttonSelectors = ['#command-submit', '#submit-btn', '.command-submit-btn', 'button[type="submit"]'];
      ...
      const executeCommand = () => {
        if (commandInput) {
          ...
          // Trigger input and change events to update any listeners
          commandInput.dispatchEvent(new Event('input', { bubbles: true }));
          commandInput.dispatchEvent(new Event('change', { bubbles: true }));
          commandInput.dispatchEvent(new Event('focus', { bubbles: true }));
          ...
          setTimeout(() => {
            ...
            if (submitButton) {
              submitButton.click();
              console.log('Command submitted via button:', command);
            } else {
              ...
              console.log('Command submitted via Enter key:', command);
            }
          }, 100);
      ...
      // Try to execute the command through the UI first
      if (executeCommand()) {
        console.log(`Command executed through UI: ${command}`);
        return;
      }
      
      // Fallback 1: Try to find a form and submit it directly
      const form = document.querySelector('form');
      if (form && commandInput) {
        commandInput.value = command;
        form.dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }));
        console.log('Command submitted via form:', command);
        return;
      }
      
      // Fallback 2: Try to use the NLI API directly
      console.log('Attempting to send command directly to NLI API');
      fetch('/api/nli', {
        method: 'POST',
        headers: {
      ...
      .then(response => {
        if (!response.ok) {
          throw new Error(`NLI request failed: ${response.statusText}`);
        }
        console.log('Direct NLI API call successful');
      })
      .catch(error => {
        console.error('Error calling NLI API directly:', error);
        eventBus.emit('notification', {
          title: 'Command Error',
          message: 'Could not execute command. No valid input method found.',
          type: 'error',
          duration: 5000
        });
      });
    ...
    // Initialize YAML Maps integration
    initYamlMapsIntegration();
  ...
  /**
   * Apply all accessibility settings from localStorage
   */
  function applyAccessibilitySettings() {
    try {
      // Get stored user preferences
      const userPreferences = JSON.parse(localStorage.getItem('userPreferences') || '{}');
      const accessibilitySettings = userPreferences?.accessibility || {};
      
      console.log('[DEBUG] Applying accessibility settings on load:', accessibilitySettings);
      
      // Apply large text setting
      if (accessibilitySettings.largeText) {
        document.body.classList.add('large-text');
        console.log('[ACCESSIBILITY] Applied large text setting');
      } else {
      ...
      if (accessibilitySettings.highContrast) {
        ...
        console.log('[ACCESSIBILITY] Applied high contrast setting');
      } else {
        document.body.classList.remove('high-contrast');
      }
      
      // Apply focus outlines setting
      if (accessibilitySettings.focusOutlines) {
        document.body.classList.add('focus-outlines');
        console.log('[ACCESSIBILITY] Applied focus outlines setting');
      } else {
        document.body.classList.remove('focus-outlines');
      }
      
      // Note: notification position is handled during component init
    } catch (error) {
  ...
  function initializeAll() {
    // Apply accessibility settings before any UI is shown
