...
// HISTORY - Optimized to be non-blocking
app.get('/history', (req, res) => {
  if (!req.session.user) {
    return res.status(401).json({ error: 'Not authenticated' });
  }
  
  // Return an immediate empty response
  res.json({ items: [], success: true, loading: true });
  
  // Then load history asynchronously in the background
  // Client should implement progressive loading with a separate fetch if needed
});

// Dedicated endpoint for actual history data loading
